#!/bin/bash
# fix_main_consumer.sh

# 1. flows Feld public machen
sed -i 's/    flows: Vec<Flow>,/    pub flows: Vec<Flow>,/' src/core/node.rs

# 2. main.rs anpassen um node.flows direkt zu nutzen
cat > src/main.rs << 'EOF'
mod core;
mod config;
mod producers;
...
echo "main.rs und node.rs korrigiert. Teste..."
cargo run
