...
# Finde die tatsächlichen Bibliothekspfade
echo "Suche Bibliotheken..."
find target/debug -name "*.rlib" | grep -E "(airlift_node|anyhow)" | head -10

# Einfacher direkter Buffer-Test
cat > test_buffer_direct.rs << 'EOF'
use std::sync::Arc;
use std::thread;
...
echo "Kompiliere direkten Test..."
rustc --edition=2021 test_buffer_direct.rs -o test_buffer_direct && ./test_buffer_direct
