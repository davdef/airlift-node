...
impl Flow {
    ...
    pub fn use_simplified_pipeline(&mut self) {
        ...
        self.processor_links = self
            .processor_buffers
            .iter()
            .cloned()
            .map(|buffer| ProcessorLink {
                buffer: Some(buffer),
            })
            .collect();
    }
    ...
    fn rebuild_legacy_buffers(&mut self) {
        ...
        for link in &mut self.processor_links {
            let buffer = link
                .buffer
                .clone()
                .unwrap_or_else(|| Arc::new(AudioRingBuffer::new(1000)));
    ...
    pub fn add_input_from_registry(
        &mut self,
        ...
        let buffer = registry
            .get(buffer_name)
            .ok_or_else(|| AudioError::BufferNotFound {
                name: buffer_name.to_string(),
            })?;
        self.add_input_buffer(buffer);
        self.info(&format!(
            "Connected input buffer from registry '{}'",
            buffer_name
        ));
        Ok(())
    }

    pub fn remove_input_from_registry(
        &mut self,
        registry: &BufferRegistry,
        buffer_name: &str,
    ) -> AudioResult<()> {
        ...
        self.info(&format!(
            "Disconnected input buffer from registry '{}'",
            buffer_name
        ));
        Ok(())
    }
    ...
    pub fn add_processor_with_buffering(
        ...
        match self.pipeline_mode {
            PipelineMode::Legacy => {
                ...
                self.processor_links.push(ProcessorLink {
                    buffer: Some(buffer),
                });
            }
    ...
    pub fn start(&mut self) -> AudioResult<()> {
        ...
        for processor in &self.processors {
            thread_processors.push(Box::new(super::processor::basic::PassThrough::new(
                processor.name(),
            )));
        }

        let handle = std::thread::spawn(move || match pipeline_mode {
            PipelineMode::Legacy => {
        ...
        if successful_starts > 0 {
            self.info(&format!(
                "{} consumer(s) started successfully",
                successful_starts
            ));
        }
...
for (i, processor) in processors.iter_mut().enumerate() {
    ...
    pub fn stop(&mut self) -> AudioResult<()> {
        ...
        if successful_stops > 0 {
            self.info(&format!(
                "{} consumer(s) stopped successfully",
                successful_stops
            ));
        }
    ...
    pub fn status(&self) -> FlowStatus {
        ...
        let input_buffer_levels: Vec<usize> = self.input_buffers.iter().map(|b| b.len()).collect();
...
let event = Event::new(event_type, priority, "AirliftNode", "main", payload)
    ...
    pub fn add_producer(&mut self, producer: Box<dyn super::Producer>) -> AudioResult<()> {
        ...
        self.info(&format!(
            "Added producer '{}' (buffer: '{}')",
            producer_name, buffer_name
        ));
        Ok(())
    }
    ...
    pub fn connect_registered_buffer_to_flow(
        &mut self,
        buffer_name: &str,
        flow_index: usize,
    ) -> AudioResult<()> {
    ...
    pub fn connect_producer_to_flow(
        &mut self,
    ...
    #[deprecated(note = "Use connect_flow_input instead.")]
    pub fn connect_registry_to_flow(
        &mut self,
        flow_index: usize,
        buffer_name: &str,
    ) -> AudioResult<()> {
    ...
    pub fn create_and_add_mixer(
        &mut self,
        flow_index: usize,
        name: &str,
        config: crate::processors::MixerConfig,
    ) -> AudioResult<()> {
        if flow_index < self.flows.len() {
    ...
    pub fn start(&mut self) -> AudioResult<()> {
        ...
        #[cfg(feature = "debug-events")]
        self.publish_event(
            EventType::Debug(DebugEventType::NodeStarted),
            EventPriority::Info,
            serde_json::json!({
                "timestamp": crate::core::timestamp::utc_ns_now(),
        ...
        let producer_names: Vec<String> = self
            .producers
            .iter()
        ...
        if successful_starts > 0 {
            self.info(&format!(
                "{} producer(s) started successfully",
                successful_starts
            ));
        }
    ...
    pub fn stop(&mut self) -> AudioResult<()> {
        ...
        #[cfg(feature = "debug-events")]
        self.publish_event(
            EventType::Debug(DebugEventType::NodeStopped),
            EventPriority::Info,
            serde_json::json!({
                "timestamp": crate::core::timestamp::utc_ns_now(),
        ...
        if successful_producers > 0 {
            self.info(&format!(
                "{} producer(s) stopped successfully",
    ...
    pub fn status(&self) -> NodeStatus {
        ...
        let flow_status: Vec<FlowStatus> = self.flows.iter().map(|f| f.status()).collect();
...
mod tests {
    ...
    fn test_node_connect_registered_buffer_to_flow() {
        ...
        flow.add_input_from_registry(&registry, "producer:test")
            .unwrap();
