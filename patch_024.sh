...
# 2. Node add_consumer fixen (mut consumer)
sed -i 's/pub fn add_consumer(&mut self, consumer: Box<dyn Consumer>)/pub fn add_consumer(&mut self, mut consumer: Box<dyn Consumer>)/' src/core/node.rs

# 3. Consumer vor push loggen
sed -i '47,49c\        let consumer_name = consumer.name().to_string();\
        consumer.attach_input_buffer(self.output_buffer.clone());\
        self.consumers.push(consumer);\
        log::info!("Flow '\''{}'\'': Added consumer '\''{}'\''", self.name, consumer_name);' src/core/node.rs

# 4. Mixer unused variable fixen
sed -i 's/fn process(&mut self, input_buffer: &AudioRingBuffer, output_buffer: &AudioRingBuffer)/fn process(&mut self, _input_buffer: &AudioRingBuffer, output_buffer: &AudioRingBuffer)/' src/processors/mixer.rs

# Build testen
echo "Fixing done. Building..."
cargo build
