...
# 1. Abhängigkeiten prüfen/hinzufügen
cat > Cargo.toml << 'EOF'
[package]
name = "airlift-node"
version = "0.2.0"
...
# 2. ALSA-Scanner implementieren
cat > src/producers/alsa/scanner.rs << 'EOF'
use anyhow::{Context, Result};
use alsa::{pcm, Direction, device_name::HintIter, card};
...
impl DeviceScanner for AlsaDeviceScanner {
    fn scan_devices(&self) -> Result<Vec<AudioDeviceInfo>> {
        ...
        let hints = HintIter::new(None, "pcm")
        ...
        for hint in hints {
            ...
            let device_type = match direction {
                ...
                None => continue, // Skip if direction unknown
            };
            
            // Get supported formats by trying to open device
            let supported_formats = self.get_supported_formats(&name).unwrap_or_default();
            ...
            devices.push(AudioDeviceInfo {
                ...
                supported_formats,
                default_format: None,
                max_channels: self.get_max_channels(&name).unwrap_or(2),
    ...
    fn test_device(&self, device_id: &str, test_duration_ms: u64) -> Result<DeviceTestResult> {
        ...
        let pcm = pcm::PCM::new(device_id, Direction::Capture, false)
            .with_context(|| format!("Failed to open device: {}", device_id))?;
        
        let hw_params = pcm.get_hw_params_current()?;
        ...
        Ok(DeviceTestResult {
            ...
            estimated_latency_ms: Some(10.0), // Rough estimate
            warnings,
            errors: Vec::new(),
        })
    }
...
impl AlsaDeviceScanner {
    fn get_supported_formats(&self, device_id: &str) -> Result<Vec<AudioFormat>> {
        ...
        if let Ok(pcm) = pcm::PCM::new(device_id, Direction::Capture, false) {
            if let Ok(hw_params) = pcm.get_hw_params_current() {
                // Common sample rates
...
# 3. Main für Discovery erweitern
cat > src/main.rs << 'EOF'
mod core;
mod config;
mod producers;
...
fn run_discovery() -> anyhow::Result<()> {
    ...
    let scanner = producers::alsa::scanner::AlsaDeviceScanner;
...
pub struct AlsaProducer {
    ...
    samples_processed: std::sync::Arc<std::sync::atomic::AtomicU64>,
    config: crate::config::ProducerConfig,
}

impl AlsaProducer {
    pub fn new(name: &str, config: &crate::config::ProducerConfig) -> anyhow::Result<Self> {
        Ok(Self {
            name: name.to_string(),
            running: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)),
            samples_processed: std::sync::Arc::new(std::sync::atomic::AtomicU64::new(0)),
            config: config.clone(),
        })
    }
}
...
impl crate::core::Producer for AlsaProducer {
    ...
    fn status(&self) -> crate::core::ProducerStatus {
        crate::core::ProducerStatus {
            ...
            connected: false,
            samples_processed: self.samples_processed.load(std::sync::atomic::Ordering::Relaxed),
            errors: 0,
        }
    }
}
