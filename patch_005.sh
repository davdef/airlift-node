# 1. Aufräumen - doppelte Dateien entfernen
rm src/producers/alsa.rs
rm -rf src/producers/alsa/  # Altes Verzeichnis
mkdir -p src/producers/alsa

# 2. Cargo.toml korrigieren (toml crate hinzufügen)
cat > Cargo.toml << 'EOF'
[package]
name = "airlift-node"
version = "0.2.0"
...
# 3. ALSA Scanner implementieren (neu)
...
# 4. ALSA Scanner Implementierung
...
# 5. Main korrigieren (Typannotation)
cat > src/main.rs << 'EOF'
mod core;
mod config;
mod producers;
